...
class _DatePickerDialogState extends State<HijriDatePickerDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    final Widget picker = Flexible(
      child: SizedBox(height: _kMaxDayPickerHeight, child: _buildPicker()),
    );
    ...
    final Widget dayButton = IgnorePointer(
      ...
      child: _DateHeaderButton(
        ...
        child: Semantics(
          ...
          child: Text(
            hSelectedDate.toFormat("DD,dd MMMM"),
            style: dayStyle,
          ),
        ),
      ),
...
class _HijriMonthPickerState extends State<HijriMonthPicker> {
  ...
  void didUpdateWidget(HijriMonthPicker oldWidget) {
    ...
    if (!widget.selectedDate.isAtSameMomentAs(
      oldWidget.selectedDate.hYear,
      oldWidget.selectedDate.hMonth,
      oldWidget.selectedDate.hDay,
    )) {
      final int monthPage = _monthDelta(widget.firstDate, widget.selectedDate);
      _dayPickerController = PageController(initialPage: monthPage);
      _handleMonthPageChanged(monthPage);
  ...
  void _updateCurrentDate() {
    ...
    Duration timeUntilTomorrow = tomorrow
        ...
        ); //tomorrow.difference(_todayDate);
  ...
  void _handleNextMonth() {
    if (!_isDisplayingLastMonth) {
      SemanticsService.announce(
        (_nextMonthDate.hMonth.toString()),
        textDirection,
      );
  ...
  void _handlePreviousMonth() {
    if (!_isDisplayingFirstMonth) {
      SemanticsService.announce(
        (_previousMonthDate.hMonth.toString()),
        textDirection,
      );
  ...
  bool get _isDisplayingFirstMonth {
    return !_currentDisplayedMonthDate.isAfter(
      widget.firstDate.hYear,
      widget.firstDate.hMonth,
      widget.firstDate.hDay,
    );
  }
  ...
  bool get _isDisplayingLastMonth {
    return !_currentDisplayedMonthDate.isBefore(
      widget.lastDate.hYear,
      widget.lastDate.hMonth,
      widget.lastDate.hDay,
    );
  }
...
class _MonthPickerSortKey extends OrdinalSortKey {
  static const _MonthPickerSortKey previousMonth = _MonthPickerSortKey(
    1.0,
  );
...
Future<HijriCalendar?> showHijriDatePicker({
  ...
  assert(
    !initialDate.isAfter(lastDate.hYear, lastDate.hMonth, lastDate.hDay),
