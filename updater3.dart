...
void main() {
  ...
  int startFilterIdx = -1;
  int buildStartIdx = -1;
  int bodyColIdx = -1;
  int entriesListIdx = -1;

  for (int i = 0; i < lines.length; i++) {
    if (lines[i].contains('Widget _buildFilterChip(') && startFilterIdx == -1) {
      startFilterIdx = i;
    }
    if (lines[i].contains('@override') &&
        i + 1 < lines.length &&
        lines[i + 1].contains('Widget build(') &&
        buildStartIdx == -1) {
      buildStartIdx = i;
    }
    if (lines[i].contains('      body: Column(') && bodyColIdx == -1) {
      bodyColIdx = i;
    }
  ...
  if (importIndex != -1 &&
    ...
    // shift everything by 1 because of import insert
    startFilterIdx++;
    buildStartIdx++;
    bodyColIdx++;
    entriesListIdx++;
  ...
  Widget _buildActiveFiltersRow(AllEntriesState state, AllEntriesNotifier notifier) {
    ...
    if (state.writerType != null) {
      activeFilters.add(_buildActiveFilterChip(state.writerType!, () => notifier.updateFilters(clearWriterType: true)));
    }
...
''';
    ...
    final beforeFilters = lines.sublist(0, startFilterIdx);
    final afterFiltersBeforeBody = lines.sublist(
      buildStartIdx,
      bodyColIdx + 2,
    ); // Includes 'body: Column(' and 'children: ['

    final bodyReplacements = [
      '          _buildSearchBarAndFilterIcon(context, ref, notifier, entriesState),',
      '          _buildActiveFiltersRow(entriesState, notifier),',
    ];

    final afterBody = lines.sublist(entriesListIdx);

    final finalLines = [
      ...beforeFilters,
      newContent,
      ...afterFiltersBeforeBody,
      ...bodyReplacements,
      ...afterBody,
    ];

    file.writeAsStringSync(finalLines.join('\\n'));
    print('AllEntriesTab Replacement Success');
  } else {
    print(
      'Failed to find indices: import=\$importIndex, startFilter=\$startFilterIdx, buildStart=\$buildStartIdx, bodyCol=\$bodyColIdx, entriesList=\$entriesListIdx',
    );
  }
}
